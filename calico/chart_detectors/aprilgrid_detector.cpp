...
void AprilGridDetector::SetupDetector() {
  // None of these parameters really need to change. Kalibr's aprilgrid
  // generator doesn't explicitly support other tag families and requires a
  // 2-bit black border.
  ...
  for (int row = 0; row < config_.tagRows; ++row) {
    for (int col = 0; col < config_.tagCols; ++col) {
      ...
      for (int k = 0; k < 4; ++k) {
        ...
        model_definition_[feature_id] =
            Eigen::Vector3d(corner_x, corner_y, 0.0);
      }
    }
  }
}

std::unordered_map<int, Eigen::Vector2d> AprilGridDetector::Detect(
  ...
  for (const auto& detected_tag : detected_tags) {
    ...
    if (tag_id < config_.startId ||
        tag_id >= config_.startId + config_.tagCols * config_.tagRows) {
      continue;
    }
    for (int i = 0; i < 4; ++i) {
